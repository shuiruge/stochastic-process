...
<\body>
  ...
  * Grassmann Variable, Berezin Integral, and Ghosts>
  ...
  This phenomenon appears everywhere in Nature, generally in a group of
  individuals. These groups, if sufficiently simplified, can be seen as a
  Langevin process. In addition, since each individual in the group is
  identical (this is true for birds, but not precise for ants), it is
  plausible to assume that the covariance is constant. So, we can describe
  such groups using action <reference|equation:langevin action constant
  covariance>. In this section, we are to investigate how far will
  information propagate in a Langevin process with constant covariance.

  First of all, we have to clarify what is the information and how to
  determine the distance it has propagated. In 2010, a group of Italian
  physicists studied the correlations in starling flocks.<\footnote>
  ...
  </footnote> They were equipped with high precision cameras, and built a
  computer software that automatically recognizes each bird in the flock,
  tracing their position in each frame. They analyzed the data and found that
  starling flocks are scale-free. That is, any two birds in the same flock
  are correlated, no matter how large the flock is.
</body>
