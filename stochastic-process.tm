...
<\body>
  ...
  It has fixed point at <math|u<rsub|\<star\>>=<around*|(|3u<rsub|\<star\>>+v<rsub|\<star\>>|)>/2>
  and <math|v<rsub|\<star\>>=<around*|(|3v<rsub|\<star\>>+u<rsub|\<star\>>|)>/2>,
  which have solution <math|u<rsub|\<star\>>=-v<rsub|\<star\>>>. This fixed
  point, however, is not stable. As it is shown in figure <reference|figure:
  vector field of differences>, a tiny departure from the line <math|u+v=0>
  will push the <math|<around*|(|u,v|)>> away from the fixed point. So, a
  linear system cannot arrive at the fixed point, thus cannot be
  scale-invariant.
