...
from sptools import Langevin, Fisher
...
def test_chain(T=1e+0):
    ...
    fisher = Fisher(Langevin(dynamics, temperature=1e-4))
...
def test_folk_bifurcation(T=1e+1):
    ...
    fisher = Fisher(Langevin(dynamics))
...
def test_limit_circle(param_type='mu', T=1e+1):
    ...
    fisher = Fisher(Langevin(
        dynamics, temperature=1e-4, absolute_tolerance=1e-2,
        moderate_stepsize=1e-1, relative_tolerance=2e-1))
    init_x = np.zeros([100, 2])

    params, fisher_vals = [], []
    if param_type == 'mu':
        raw_params = np.linspace(0.05, 5, 20)
    else:
        raw_params = np.linspace(0, 10, 20)
    for param in tqdm(raw_params):
        try:
            fisher_val, final_x = fisher(param, init_x, T)
            params.append(param)
            fisher_vals.append(fisher_val)
        except StopIteration:
            print(f'Cannot process param: {param}')
            pass
...
def test_lorenz(r_max=40, T=1e+0):
    def dynamics(r, x):
        sigma = 10
        b = 8/3
        f = [
    ...
    params, fisher_vals = [], []
    raw_params = np.linspace(0, r_max, 25)
    for param in tqdm(raw_params):
        try:
            fisher_val, final_x = fisher(param, init_x, T)
            params.append(param)
